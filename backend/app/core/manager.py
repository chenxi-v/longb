...
class SpiderManager:
    ...
    def load_spider(self, key: str, path: str, spider_type: str, extend: str = "") -> Spider:
        ...
        if key in self.spiders:
            logger.debug(f"爬虫已加载，直接返回: {key}")
            return self.spiders[key]
        
        # 获取加载锁
        lock = self._get_loading_lock(key)
        
        # 双重检查锁定
        with lock:
            if key in self.spiders:
                logger.debug(f"爬虫已加载（双重检查），直接返回: {key}")
                return self.spiders[key]
            
            logger.info(f"开始加载爬虫: {key} (类型: {spider_type})")
            ...
            try:
                ...
                logger.info(f"爬虫加载成功: {key}")
                return spider
                
            except Exception as e:
                logger.error(f"爬虫加载失败: {key} - {e}")
                raise
